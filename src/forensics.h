...
typedef struct forensics_breadcrumb_t {
  const wchar_t* name;         // the name of this breadcrumb
  const wchar_t** meta_keys;   // array of metadata key strings
  const wchar_t** meta_values; // array of metadata value strings
...
typedef struct forensics_report_t {
  const wchar_t* id;        // A agrregation id (or fingerprint) for this report: "CONTEXT-FILE_BASENAME-FUNC-MSG_FORMAT_STRING"
  ...
  const wchar_t* format;    // The format string (i.e. the unformatted message).
  const wchar_t* formatted; // The formatted message.
  ...
  const wchar_t* const* context_stack; // The stack of error contexts. The most recent (i.e. responsible one) is at the end.
  int context_count;                   // The number of contexts on the stack

  const wchar_t* const* attribute_keys;   // Array of attribute key strings
  const wchar_t* const* attribute_values; // Array of attribute value strings
...
#define FORENSICS_JOIN_IMPL(a, b) a##b
#define FORENSICS_JOIN(a, b) FORENSICS_JOIN_IMPL(a, b)
#define FORENSICS_WCHAR_IMPL(str) L##str
#define FORENSICS_WCHAR(str) FORENSICS_WCHAR_IMPL(str)

// A fatal assertion with no message.
#define FORENSICS_ASSERT(expr) ((expr) ? true : (forensics_report_assert_failure(__FILE__, __LINE__, __func__, true, #expr, L""), false))
...
#define FORENSICS_VERIFY(expr) ((expr) ? true : (forensics_report_assert_failure(__FILE__, __LINE__, __func__, false, #expr, L""), false))
...
#define FORENSICS_ASSERT_DBG(expr) ((expr) ? true : (forensics_report_assert_failure(__FILE__, __LINE__, __func__, true, #expr, L""), false))
...
#define FORENSICS_CONTEXT(name) forensics_context_t FORENSICS_JOIN(forensics_context__, __LINE__)(name)
...
typedef struct forensics_context_t {
  inline forensics_context_t(const wchar_t* name) {
...
// #undef FORENSICS_JOIN_IMPL
// #undef FORENSICS_JOIN
// #undef FORENSICS_WCHAR
